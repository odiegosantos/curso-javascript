...
function filtrarCarros() {
  ...
  var NaoHaCarros = `Não há carros com preço até R$: ${maximo.toFixed(2)}`;
  var carrosAte = `Carros até R$: ${maximo.toFixed(2)} \n-------------------------\n ${lista}`;

  if (lista == "") {
    outLista.textContent = `${NaoHaCarros}`;
  } else {
    outLista.textContent = `${carrosAte}`;
  }
}
